...
from adapteacher.modeling.GModule.multi_graph_matching import MGM3_unsup, U_sup
...
class DAobjTwoStagePseudoLabGeneralizedRCNN(GeneralizedRCNN):
    ...
    def __init__(
        ...
        *,
        ...
        sample_dist = 10 # d:10
        univ_size = 32
        # graph_opt.MODEL.FCOS.NUM_CLASSES = cfg.MODEL.ROI_HEADS.NUM_CLASSES + 1
        self.graph_generator = PrototypeComputation(self.roi_heads.num_classes, sample_dist)
        self.multi_matching_sup = U_sup(self.roi_heads.num_classes, univ_size) #.to(self.device)
        self.multi_matching_unsup = MGM3_unsup(self.roi_heads.num_classes, univ_size)

    def build_discriminator(self):
        self.D_img = FCDiscriminator_img([self.dis_type]).to(self.device) # Need to know the channel
    ...
    def forward(
        ...
        if branch.startswith("supervised_source"):
            ...
            # targets = [x['instances'] for x in gt_instances]
            features = [feat[1] for feat in features.items()]
            
            nodes, labels = self.graph_generator(features, gt_instances)
            loss_matching = self.multi_matching_sup(nodes, labels) # 
            losses.update({'loss_matching': loss_matching})
            ...
            # )
            features = [feat[1] for feat in features.items()]
            nodes, labels = self.graph_generator(features, proposals_roih)
            U = self.multi_matching_sup.U
            loss = self.multi_matching_unsup(nodes, labels, U)
            # draw_tensor(U)
...
if __name__ == '__main__':
    ...
    multi_matching_sup = U_sup(2, 32).to("cuda:1")
    multi_matching_unsup = MGM3_unsup(2, 32).to("cuda:1")

    # ----------FLOPs and Param --------------
    total_params = sum(p.numel() for p in multi_matching_sup.parameters())
    trainable_params = sum(p.numel() for p in multi_matching_sup.parameters() if p.requires_grad)
    print(f"Total Parameters: {total_params}")
    print(f"Trainable Parameters: {trainable_params}")

    total_params = sum(p.numel() for p in multi_matching_unsup.parameters())
    trainable_params = sum(p.numel() for p in multi_matching_unsup.parameters() if p.requires_grad)
    print(f"Total Parameters: {total_params}")
    print(f"Trainable Parameters: {trainable_params}")

    # calculate unsupervised (test-time)
    class GModuleWrapper(torch.nn.Module):
        def __init__(self, model):
            super(GModuleWrapper, self).__init__()
            self.model = model
        ...
        def forward(self, inputs):
            ...
            labels = inputs[1]
            x = inputs[2]
            return self.model(feats, labels, x)
        
    multi_matching_unsup_wrapped = GModuleWrapper(multi_matching_unsup)
    labels = []
    feats = []
    for i in range(4):
        labels.append(torch.randint(0, 2, (40,), dtype=torch.int).to("cuda:1"))
        feats.append(torch.randn(40, 256).to("cuda:1"))
    x = torch.randn(32, 256).to("cuda:1")
    inputs = [feats, labels, x]
    flops = FlopCountAnalysis(multi_matching_unsup_wrapped, inputs)
    print("Total FLOPs:", flops.total())

def draw_tensor(tensor):
    tensor = (tensor - tensor.min()) / (tensor.max() - tensor.min())
    tensor = tensor * 255  # 转换为 [0, 255]
    tensor = tensor.byte()
    # plt.imshow(tensor.numpy(), cmap='gray')  # 使用灰度色图
